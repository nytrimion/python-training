...
from src.module_04_integration.infrastructure.celery_job_dispatcher import (
    CeleryJobDispatcher,
)
...
class TestCeleryJobDispatcher:
    ...
    @pytest.fixture
    def _mock_send_task(self, mocker):
        mock_task = Mock()
    ...
    def test_dispatch_sends_task_with_payload(self, _mock_send_task):
        """Test dispatching task with payload."""
        self.dispatcher.dispatch("job", DummyPayload(data={"foo": "bar"}))

        _mock_send_task.assert_called_once_with("job", args=[{"foo": "bar"}])

    def test_dispatch_returns_job_with_expected_properties(self, _mock_send_task):
    ...
    def test_dispatch_uses_registered_task_name(self, _mock_send_task):
        """Test dispatch uses registered task name."""
        self.dispatcher.register("job", "task")
        self.dispatcher.dispatch("job", DummyPayload())

        _mock_send_task.assert_called_once_with("task", args=ANY)

    def test_dispatch_uses_job_name_as_default_task_name(self, _mock_send_task):
        """Test dispatch uses job name as default task name."""
        self.dispatcher.dispatch("job", DummyPayload())

        _mock_send_task.assert_called_once_with("job", args=ANY)
